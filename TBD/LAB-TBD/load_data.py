...
def get_connection_db(dbName):
	return psycopg2.connect(user="postgres", password="clavexd", host="127.0.0.1", port="5432", database="testSerial")
...
def main(tables,dbName,swDelete,swInsert):
	...
	if(swInsert=='y'):
		for table in tables:
			i=0
			#Borrar el id de las tuplas
			while i<len(table["record_to_insert"]):
				table["record_to_insert"][i].pop(0)
				i=i+1
			table["postgres_insert_query"] = table["postgres_insert_query"].replace('(id,','(')
			table["postgres_insert_query"] = table["postgres_insert_query"].replace('(%s,','(')
			table["columns"].pop(0)
			table["columnsType"].pop(0)
			table["data"].pop("id")
